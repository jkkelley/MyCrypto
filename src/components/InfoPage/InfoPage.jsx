...
const coinsSort = coinsFromGecko.filter(coins => 
  ...
  useEffect(() => {
    axios.get("https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=100&page=1&sparkline=false&price_change_percentage=1h%2C%2024h%2C%207d%2C%2030d%2C%201y")
    .then(response => {
  ...
  return (
    ...
    {coinsSort.map((coins) => {
      return (<InfoPage2 key={coins.id} name={coins.name} price={coins.current_price}/>)
    })}
